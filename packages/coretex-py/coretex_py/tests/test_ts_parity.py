...
coretex-py is the dev-only cross-impl harness; packages/coretex/src is the
...
from coretex_py.types import PATCH_TYPE, RANGES

_TS_TYPES_PATH = os.path.join(
    os.path.dirname(__file__),
    "..", "..", "..", "coretex", "src", "state", "types.ts",
)

requires_ts = pytest.mark.skipif(
    not os.path.exists(_TS_TYPES_PATH), reason="packages/coretex/src not present"
)
