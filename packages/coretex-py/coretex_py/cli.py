"""
cli.py — Command-line interface for coretex_py.

Usage:
  python -m coretex_py merkleize <hex_state_bytes>
  ...
  python -m coretex_py apply-patch <state_hex> <patch_hex>
      Applies the patch to the state.
      Prints JSON: {"ok": true, "state": "<hex>"} or {"ok": false, "code": "EXX"}

  python -m coretex_py batch-stdin
  ...
  python -m coretex_py verify-keccak
...
def main() -> None:
    ...
    if cmd == "merkleize":
        if len(args) != 2:
            print("Usage: coretex_py merkleize <hex_state_bytes>", file=sys.stderr)
            sys.exit(1)
        print(_merkleize_hex(args[1]))

    elif cmd == "apply-patch":
        if len(args) != 3:
            print("Usage: coretex_py apply-patch <state_hex> <patch_hex>", file=sys.stderr)
            sys.exit(1)
