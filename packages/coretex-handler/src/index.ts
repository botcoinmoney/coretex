/**
 * @botcoin/coretex-handler — single-line drop-in router for the SWCP coordinator.
 *
 ...
 *
 *   import { mountCortexHandler } from '@botcoin/coretex-handler';
...
import { getCortexStore } from './coretex-store.js';
...
export interface CortexHandlerDeps {
  ...
  /** Optional: path to the coretex-store SQLite DB. Defaults to CORTEX_STORE_DB_PATH env. */
...
export function mountCortexHandler(app: MinimalApp, deps: CortexHandlerDeps): void {
  ...
  app.post('/internal/sign-cortex-receipt', (req, res) => {
    void (async () => {
      ...
      try {
        ...
        console.error('[coretex-handler] sign-cortex-receipt failed:', err);
  ...
  app.post('/internal/sign-coretex-work-receipt', (req, res) => {
    void (async () => {
      ...
      try {
        ...
        console.error('[coretex-handler] sign-coretex-work-receipt failed:', err);
        sendJson(res, 500, { error: 'signing-failed', detail: String(err) });
      }
    })();
  });
  ...
  console.log('[coretex-handler] mounted /internal/* routes on SWCP coordinator');
}

export { getCortexStore } from './coretex-store.js';
export type { CortexStore, OutstandingChallengeRecord, MergeBonusFundingRecord } from './coretex-store.js';
