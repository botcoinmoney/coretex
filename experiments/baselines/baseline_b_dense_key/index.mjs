...
// of the next uncovered near-collision event so the active key matches the
// corpus.
...
export function mineCandidatePatch(state, shardDescriptor, opts = {}) {
  ...
  if (!corpus) {
    ...
    return makePatch(PATCH_TYPE.KEY_UPDATE, [idx], [newWord], 100n, new Uint8Array(32));
  }
  ...
  while (indices.length < 4 && probesUsed < events.length && slotsUsed < 36) {
    ...
    for (let i = 0; i < 36; i++) {
      ...
      if (slotIds[cand] === 0n) { slotIdx = cand; break; }
    }
    if (slotIdx < 0) {
      // No empty slot — pick the next slot in rotation that isn't already
      // queued in this patch.
      for (let i = 0; i < 36; i++) {
        const cand = (slotProbe + i) % 36;
