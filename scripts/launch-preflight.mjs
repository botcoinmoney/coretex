 ...
 *
 * Usage:
 *   node scripts/launch-preflight.mjs                 # local coherence + emit fingerprint
...
const PROFILE = 'release/bundle/evaluator-profile-v2-dgen1-policy-r5-300k.json';
const BUNDLE = 'release/bundle/bundle-manifest-v2-dgen1-policy-r5-300k.json';
...
console.log('══════════ LAUNCH PREFLIGHT (300k canonical tree) ══════════');
...
const corpusRoot = computeCorpusRoot(corpus.events);
...
const rawK = '0x' + createHash('sha256').update('x').digest('hex'); // placeholder presence check
