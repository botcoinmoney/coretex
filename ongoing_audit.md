...
## Orchestrator handoff round 6 (2026-05-10, real-model calibration)

This CPU instance ran the full calibration pipeline end-to-end with
real Qwen3-Reranker-0.6B + BAAI/bge-m3 + IAAR-Shanghai/MemReranker-4B
weights, on this host's hardware. Toolchain provisioned: Node 22.22.2,
Foundry 1.7.1, Python 3.12 venv with torch 2.6.0+cpu, transformers
4.55.0, huggingface_hub 0.36.2, tokenizers 0.21.4, sentence_transformers
3.4.1. Pinned model files (32 total: 12 BGE-M3, 12 Qwen3, 11 MemReranker)
downloaded via `scripts/download-pinned-models.py` with
`huggingface_hub.hf_hub_download` and re-verified against the per-file
SHA-256 + byte-length hashes pinned in `packages/cortex/src/bundle/index.ts`.
12 GB total model cache.
...
- The legacy bi-encoder + reranker factories spawned a fresh Python
  ...
  `createStreamingBiEncoder` + `createStreamingQwen3Reranker` keep one
  long-lived child per pinned model and service NDJSON requests over
  stdin/stdout with correlation-id multiplexing. Both runners now use
  truly batched padded forward passes; per-pair logits are read at
  the actual last non-pad position so batched scores are bit-identical
  to single-pair scores. Verified on this host: 0 ppm cross-run diff
  on 200 pairs across 3 logical hosts. (commit b9bd720)
- `rerankerFromEnv` honors `CORETEX_RERANKER_MODE=streaming` so the
  determinism harness, calibrator, and Phase 13 e2e all use the
  persistent path. (commit fb2a708)
- Parallel-shard driver `scripts/generate-coretex-retrieval-corpus-parallel.mjs`
  dispatches disjoint seed-offset shards and merges deterministically.
  (commit 9e40045)
- Bundle manifest `runtimePin` defaults bumped from torch 2.4 / transformers
  4.46 to torch 2.6 / transformers 4.55 / huggingface_hub 0.36 / tokenizers
  0.21, because transformers ≥ 4.51 is required to load Qwen3 and
  transformers ≥ 4.55 raises a torch-version guard requiring torch ≥ 2.6
  for any pickle-encoded checkpoint (BGE-M3 still ships
  `pytorch_model.bin`). `matchSemverRange` now strips PEP-440 build
  metadata (`+cpu`) before matching. (this commit)

Calibration outputs (canonical, ship-deployable):
...
corpus events    1752
corpus splits    calibration:166 / train_visible:1241 / eval_hidden:266 / canary:79
corpus families  near_collision:992 / temporal:120 / long_horizon:571 / multi_hop_relation:69
determinism p99  0 ppm cross-run on 400 pair diffs (max tolerance 250 ppm)
replayTolerance  250 ppm (well below minImprovementPpm = 2500)
runtimePin       torch 2.6.* / transformers 4.55.* / huggingface_hub 0.36.* / tokenizers 0.21.*
acceleratorPolicy cpu_only
```

`assertBundleBindingAtStartup` accepts the canonical bundle on this
host given the installed runtime versions above. `verifyBundleManifest`
reports zero errors. Bundle binds the pipeline + pinned models + the
calibration corpus root.

Phase 13 e2e against real Qwen3 + BGE-M3 on the calibration corpus:
...
adv     rejected                   reason = no_retrieval_improvement
status  phase-13: PASS
```

3/5 accepted (real ndcg@10 improvements ≥ 2500 ppm), 2/5 rejected with
spec-correct reasons (insufficient improvement; family catastrophic
floor), adversarial correct-id-bad-vector patch refused as designed.

Why the calibration corpus is at `seeds-per-domain=4` (1752 events)
rather than the launch `seeds-per-domain=512`: MemReranker-4B labeling
is the dominant CPU cost — single-host empirical throughput on this
32-core x86_64 box is ~1.7 pair/s peak, ~0.5 pair/s on real-length
prompts (memory-bandwidth bound; parallel workers add only ~8% on a
single memory controller). The launch corpus (~679 k events × ~4 hard
negatives) is multi-week wall on this hardware. The calibration corpus
exercises the full real-model pipeline end-to-end at a tractable size
on this CPU host; the launch corpus is generated by the same scripts
unchanged on hardware where MemReranker-4B inference is faster, and the
bundle binds the pipeline + pinned models, not the host. The
`docs/CORETEX_FINAL_PRODUCTION_E2E_ORCHESTRATOR_RUNBOOK.md` Phase 1 was
updated this round to make this realistic-runtime explicit.

Mainnet read-only verification (Base mainnet, chain 8453, block
45,802,289):

```
CortexState.rewardLane()       = 0x12ff0B47389AE6d6293d44991B0D6A27394494A4 (V4)  ✓
BotcoinMiningV4.stakeSource()  = 0xB2fbe0DB5A99B4E2Dd294dE64cEd82740b53A2Ea (V3)  ✓
BotcoinMiningV4.cortexState()  = 0x5d3B9D9b246cf8457F320Bb27f008186B69D555d        ✓
BotcoinMiningV4.owner()        = 0x6463f89F102e9f53168ABe557173f53c0bBbF635        ✓
BotcoinMiningV4.coordinatorSigner() = 0x6463f89F102e9f53168ABe557173f53c0bBbF635   ✓
BotcoinMiningV4.currentEpoch() = 79
```

All four canonical addresses match `docs/contract-addresses-mainnet.md`.
Epoch advanced from 78 (deploy) to 79 (current); CoreTex epoch
rotation (Phase 5) will pin the canonical `bundleHash` above as the
`coreVersionHash` for the next CoreTex epoch.

A full canonical record of this round's bundle, profile, determinism,
Phase 13 result, capacity gate, and reproduction commands is at
`docs/CORETEX_CALIBRATION_2026-05-10.md`. The bundle artifact itself is
not committed (it lives at `/etc/coretex/bundle-manifest.json` on the
host that runs the canonical pipeline) — every value in the canonical
record is reproducible byte-identically by re-running the scripts on a
host that satisfies the same model SHA-256s + runtime pin.

Self-leak note for the audit log: in the early part of this round I
sourced `/root/cortex/.env` from bash, which printed the value of an
env var whose name began with a digit (`8004_SIGNING_KEY=...`) into the
session transcript. The user was made aware in real time and confirmed
they would handle key rotation; I renamed the var to
`BOTCOIN_8004_SIGNING_KEY` and switched to a Python-based env loader
(`scripts/run-with-env.py`) that never echoes values to stdout/stderr.
The leak does not change anything about the canonical CoreTex tree: no
secrets are tracked in git, `git ls-files` carries zero `.env`-class
material, and the leaked key is the user's coordinator-side artifact
not part of the CoreTex bundle.
